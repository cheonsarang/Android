...
import com.example.and10_inntentresult.LoginActivity;
import com.example.and10_inntentresult.LoginDTO;
import com.example.and10_inntentresult.R;
import com.example.and10_inntentresult.TestActivity;

import java.util.ArrayList;

public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    Button btn, btn_call, btn_internet, btn_gps;
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == REQ_TESTCODE){
            ...
            String a = reIntent.getStringExtra("test");
            Log.d("됐어?", ": " + a);
    ...
    public void onClick(View v) {
        if(v.getId() == R.id.btn_call) {
            //안드로이드에서 기본적으로 제공하고 우리가 만들지 않은 작업을 하게되면 암시적 인텐드라고 한다
            //전화걸기 (권한 매우 낮음 -> 매니페스트 파일에 명시만 해 놓으면 바로 사용가능한 부분) permission
            Intent intent = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:/119"));
            startActivity(intent);

        }else if(v.getId() == R.id.btn_gps) {
            //gps <= 권한을 승인받아야 함, 현재 위치를 노출해야 할 때
            //현재 위치를 얻어오기 위해서는 다른 권한이 필요함
            Intent intent = new Intent(Intent.ACTION_VIEW
            , Uri.parse("https://www.google.com/maps/?q=" + 35.1535583 + "," + 126.8879957 + "&z=" + 10)
            );
            startActivity(intent);
